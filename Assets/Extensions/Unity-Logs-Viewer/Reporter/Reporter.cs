...
public class Reporter : MonoBehaviour
{
    ...
    [SerializeField] private GameObject fpsTextObject;
    ...
    [ShowInInspector, ReadOnly] private int numOfCircleToShow = 1;
    ...
    void Awake()
    {
        ...
        if (forceEnable)
        {
            ...
            if (!Initialized)
                Initialize();
            Debug.LogWarning("Reporter numOfCircleToShow initialized " + numOfCircleToShow);
        }
        else
            StartCoroutine(Wait());
#if UNITY_CHANGE3
    ...
    IEnumerator Wait()
    {
        while (DeviceManager.Instance == null || !DeviceManager.Instance.IsInit || FirebaseManager.Instance == null || !FirebaseManager.Instance.IsLoaded)
            yield return null;
        if (!forceEnable)
        {
            if (DeviceManager.Instance.IsDeveloper())
            {
                gameObject.SetActive(true);
                if (fpsTextObject != null)
                    fpsTextObject.SetActive(true);
            ...
            {
                numOfCircleToShow = 20;
                if (!Initialized)
                    Initialize();
            }
            if (!FirebaseManager.Instance.FirebaseRemoteConfig.IsFeatureEnable(FireBaseFeatureConfigConstant.IsReporterEnable))
            {
