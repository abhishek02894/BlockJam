...
namespace UnityEngine.UI.Extensions
{
    public class ScrollSnapBase : MonoBehaviour, IBeginDragHandler, IDragHandler, IScrollSnap
    {
        ...
        private Button prevButtonMono;

        [Tooltip("Button to go to the next page. (optional)")]
        public GameObject NextButton;
        private Button nextButtonMono;
        ...
        public bool _canShowlastButton; // canShowlastButton
        ...
        void Awake()
        {
            ...
            if (NextButton)
            {
                nextButtonMono = NextButton.GetComponent<Button>();
                nextButtonMono.onClick.AddListener(() => { NextScreen(); });
            }

            if (PrevButton)
            {
                prevButtonMono = PrevButton.GetComponent<Button>();
                prevButtonMono.onClick.AddListener(() => { PreviousScreen(); });
        ...
        private void ToggleNavigationButtons(int targetScreen)
        {
            ...
            if (!_isInfinate)
            {
                if (prevButtonMono)
                {
                    prevButtonMono.interactable = targetScreen > 0;
                }
                if (nextButtonMono)
                {
                    nextButtonMono.interactable = targetScreen < _screensContainer.transform.childCount - 1;
                }
            }
            if (!_canShowlastButton)
            {
                if (prevButtonMono)
                {
                    prevButtonMono.gameObject.SetActive(targetScreen > 0);
                }
                if (NextButton)
                {
                    nextButtonMono.gameObject.SetActive(targetScreen < _screensContainer.transform.childCount - 1);
                }
            }
        }
